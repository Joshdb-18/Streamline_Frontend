...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    try {
      ...
      const response = await axios.post('https://backend.devnetwork.tech/api/v1/reset_confirm/', {
        uidb64,
        token,
        password1,
        password2,
      ...
      if (response.status === 200) {
        ...
        return res.status(response.status).json({ error: 'Password update verification failed.' });
      }
    } catch (error) {
      ...
      // Return a response indicating an error
      return res.status(500).json({ error: 'An error occurred during password update verification.' });
    }
  }
